...
function Explorer:ScheduleNodeRealiser()
    ...
    task.spawn(function()
        while not KillScript do
            local Budget = self.SearchQuery ~= "" and 200 or 8
            local Created = 0
...
function Explorer:JumpToCharacter(Player)
    ...
    local Chain = {}
    local Cursor = Character
    while Cursor and Cursor ~= game do
        table.insert(Chain, 1, Cursor)
        Cursor = Cursor.Parent
    end

    local function ConsumePending(ParentNode, Target)
        if not ParentNode or not ParentNode.PendingChildren then
            return
        end
    ...
    local function RealiseAncestor(ParentNode, Target)
        if self.NodesByInstance[Target] then
            return self.NodesByInstance[Target]
        end
        ...
        ConsumePending(ParentNode, Target)

        if not self.NodesByInstance[Target] then
            self:CreateChildNode(ParentNode, Target)
        end
    ...
    for Index, Ancestor in Chain do
        ...
        if not Node then
            ...
            Node = RealiseAncestor(ParentNode, Ancestor)
        end
        ...
        if Ancestor ~= Character and not Node.Expanded then
            self:ExpandNode(Node)
        end

        local NextAncestor = Chain[Index + 1]
        if NextAncestor then
            RealiseAncestor(Node, NextAncestor)
        end
    end
...
function Explorer:CollectNilInstances(SearchFilter, ClassFilter)
    ...
    local function Consider(Instance)
        ...
        local GoodChildren = type(Children) == "table"
